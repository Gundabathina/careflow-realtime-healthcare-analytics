...
### Orchestrated Hospital Operations & Patient Readmission Analytics Platform
...
<!-- Once deployed, add: **[Live Demo](https://<your-render-service>.onrender.com)** here.
     See docs/github_setup.md#after-the-dashboard-is-deployed-manual-steps-once-a-real-url-exists
     for the exact steps -- do not fill in a URL until deployment is real. -->
...
[Security & synthetic-data note](#21-security--synthetic-data-note).
...
21. [Security / synthetic-data note](#21-security--synthetic-data-note)
...
Component-specific diagrams: [Airflow DAG task graphs](docs/architecture/airflow_workflow.md),
[dashboard architecture](docs/architecture/dashboard_architecture.md),
...
| Project test suite | **804 passed, 2 skipped** (see [Test results](#20-test-results)) |
...
(never committed -- copy it from the tracked `.env.example`, which
documents every variable with safe, clearly-labeled placeholder
defaults). None of the values below are real credentials.

| Variable | Used by | Notes |
|---|---|---|
| `POSTGRES_HOST` / `POSTGRES_PORT` / `POSTGRES_DB` / `POSTGRES_USER` / `POSTGRES_PASSWORD` | `docker-compose.yml`, `src/careflow/warehouse/postgres_client.py` | The CareFlow analytics warehouse. Change the password for anything beyond local dev. |
| `AIRFLOW_POSTGRES_DB` / `AIRFLOW_POSTGRES_USER` / `AIRFLOW_POSTGRES_PASSWORD` | `docker-compose.yml` (`airflow` profile) | Airflow's own metadata database -- a separate PostgreSQL instance/credentials from the warehouse above; never reuse `POSTGRES_PASSWORD` here. |
| `AIRFLOW_FERNET_KEY` / `AIRFLOW_WEBSERVER_SECRET_KEY` | Airflow webserver/scheduler | Generate real values for anything beyond local dev (commands in `.env.example`); never reuse the example placeholders. |
| `AIRFLOW_ADMIN_USERNAME` / `AIRFLOW_ADMIN_PASSWORD` / `AIRFLOW_ADMIN_EMAIL` | Airflow's first admin user (`airflow-init`) | Local dev only by default. |
| `AIRFLOW_UID` | Airflow containers | Match your host UID (`id -u`) so bind-mounted `dags`/`plugins`/`logs` have correct permissions. |
| `AIRFLOW_WEBSERVER_PORT` | `docker-compose.yml` | Host port the Airflow UI is mapped to (defaults to `8081` in `.env.example` to avoid clashing with another local service on `8080`). |
| `DBT_TARGET` | `profiles.yml.example`, `scripts/run_dbt.py` | Selects the dbt target/profile (`dev` by default). |
| `CAREFLOW_PROJECT_CONFIG` | `src/careflow/config.py` | Optional override for the path to `config/project_config.yaml`. |

Every credential is read through one loader
(`careflow.warehouse.postgres_client.load_connection_config()`) and every
driver exception is sanitized before logging -- see
...
- [`docs/architecture/architecture.md`](docs/architecture/architecture.md) -- overall system diagram
- [`docs/architecture/data_flow.md`](docs/architecture/data_flow.md) -- stage-by-stage ETL pipeline detail
- [`docs/architecture/warehouse_model.md`](docs/architecture/warehouse_model.md) -- dimensional model
- [`docs/architecture/airflow_workflow.md`](docs/architecture/airflow_workflow.md) -- both DAGs' task graphs
- [`docs/architecture/dashboard_architecture.md`](docs/architecture/dashboard_architecture.md) -- Streamlit component architecture
- [`docs/architecture/technology_stack.md`](docs/architecture/technology_stack.md) -- how every technology fits together, and why

**Reference**
- [`docs/api_reference.md`](docs/api_reference.md) -- CLI scripts and public Python functions
- [`docs/deployment_guide.md`](docs/deployment_guide.md) -- Docker, Streamlit Community Cloud, Render, Railway, AWS EC2
- [`docs/project_metrics.md`](docs/project_metrics.md) -- every verified metric and how to regenerate it
- [`docs/security.md`](docs/security.md) / [`docs/data_ethics.md`](docs/data_ethics.md) -- credential and PII policy

**Portfolio & recruiter material**
- [`docs/project_summary.md`](docs/project_summary.md) -- one-page overview
- [`docs/business_value.md`](docs/business_value.md) -- what this kind of platform enables
- [`docs/technical_highlights.md`](docs/technical_highlights.md) -- specific engineering problems solved
...
- [`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md) / [`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/) -- issue and PR templates
